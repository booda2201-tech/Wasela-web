...
export function normalizeCmsPage(raw: unknown): CmsPage {
  ...
  return {
    ...
    // Explicit false from API = inactive; missing field defaults to active
    isActive: (r['isActive'] ?? r['IsActive'] ?? true) !== false,
...
export class PagesService {
  ...
  getPageBySlug(slug: string): Observable<CmsPage> {
    return combineLatest([this.getRawPage(slug, false), this.languageService.lang$]).pipe(
      map(([page, lang]) => localizeCmsPage(page, lang))
    );
  }

  /** Bypass in-memory + HTTP cache — footer/contact must reflect dashboard Save immediately. */
  getPageBySlugFresh(slug: string): Observable<CmsPage> {
    this.rawPageCache.delete(slug);
    return combineLatest([this.getRawPage(slug, true), this.languageService.lang$]).pipe(
      map(([page, lang]) => localizeCmsPage(page, lang))
    );
  }

  /** Drop cached CMS pages so the next read hits the API again. */
  invalidatePageCache(slug?: string): void {
    if (slug) {
      this.rawPageCache.delete(slug);
      return;
    }
    this.rawPageCache.clear();
  }

  private getRawPage(slug: string, bustHttp: boolean): Observable<CmsPage> {
    if (!bustHttp) {
